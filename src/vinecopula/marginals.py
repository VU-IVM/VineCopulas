...
def best_fit_distributiondiscrete(data, criterion="BIC", bound=False):
    """
    ...
    for name, distribution in distributions.items():
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                if criterion == "BIC":
                    criterion = -2 * log_likelihood + np.log(n) * k
                elif criterion == "AIC":
                    criterion = -2 * k - 2 * log_likelihood
                elif criterion == "ML":
                    criterion = log_likelihood

                # identify if this distribution is better
                best_distributions.append((distribution, params, criterion))
