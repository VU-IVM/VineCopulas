...
def vinecop(u1, copsi, vine ='R', printing = True):
    ...
    for i in range(dimen-1):
        for j in range(i+1,dimen):
            ...
            tauabs.append(abs(st.kendalltau(u1[:,i], u1[:,j])[0])) # calculate the absolute kendall tau between v1 and v2 and add it to the taubs list
        
       
    order1 = pd.DataFrame({'v1': v1, 'v2': v2, 'tauabs': tauabs}  ) # put the v1, v2, and tauabs list into a dataframe for the first tree
    order1 = order1.sort_values(by='tauabs', ascending=False).reset_index(drop=True) # sort this dataframe from highest to lowest tauabs
    
    
    # R vine
    if vine == 'R':
        inde = [] # list to put used rows of order1 in 
        for i in range(len(order1)): # loop through all the rows to include those pairs with the highest ktau first
            if i == 0:
                order2 = order1.head(1) #create a new dataframe where the row with the highest ktau is the first row
            else:
                if (order1.v1[i] in list(order1.v2[:i]) or order1.v1[i] in list(order1.v1[:i]) ) and ( order1.v2[i] in list(order1.v2[:i]) or order1.v2[i] in list(order1.v1[:i])): #see if both v1i and v2i are already in order2, first rows with unique variables need to be added to ensure all variables are included
                    continue
                else:
                    inde.append(i) # add used rows to inde
                    order2 = pd.concat([order2, order1.loc[i].to_frame().T], ignore_index=True) #add row to dataframe order2
        if len(order2) < (dimen-1): #the first tree will have the number of variables - 1 number of rows. 
            for i in range(1, len(order1)): #loop through all the combinations again and add based on the highest ktauabs
                ...
                l1 = list(order2.v2[order2.v1 == order1.v2[i]]) + list(order2.v1[order2.v2 == order1.v2[i]])
                lk = l1.copy()
                while len(lk) > 0:
                    lk2 = lk.copy()
                    lk = []
                    for j in lk2:
                        ln = list(order2.v2[order2.v1 == j]) + list(order2.v1[order2.v2 == j])
                        try:
                ...
                l2 = list(order2.v2[order2.v1 == order1.v1[i]]) + list(order2.v1[order2.v2 == order1.v1[i]]) 
                lk = l2.copy()
                while len(lk) > 0:
                    lk2 = lk.copy()
                    lk = []
                ...
                if skip == True:
                  ...
                  #  continue
                if len(lst) == len(set(lst)): #check that there are no 'triangles', e.g. three variables that are alll connected with eachother 
                    order2 = pd.concat([order2, order1.loc[i].to_frame().T], ignore_index=True) #add row if conditions are met
                if len(order2) == dimen-1: #end loop when desired number of rows (number of variables - 1) has been met
                    break
    ...
    #D vine         
        ...
        for j in range(dimen - 1):
            if j == 0:
                order2 = order1.head(1)  # loop through all the rows to include those pairs with the highest ktau first
                ...
                for i in range(len(order1)):
                    if order1.v1[i] == vj or order1.v2[i] == vi or order1.v1[i] == vi or order1.v2[i] == vj: #find the rows which include at least one of the two variables from the previous edge
                        if order1.v1[i] in inde or order1.v2[i] in inde: #check if variables have already been connected to other variables twice
                            continue
                        if (order1.v1[i] == vi and order1.v2[i] == vj) or (order1.v1[i] == vj and order1.v2[i] == vi): #skip rows that have already been used
                            continue
                        else:
                            order2 = pd.concat([order2, order1.loc[i].to_frame().T], ignore_index=True) #if conditions are met, add this to the dataframe of the first tree
                            if order1.v1[i] == vj or order1.v2[i] == vj: #check which value has been used twice already
                            ...
                            elif order1.v1[i] == vi or order1.v2[i] == vi:  #check which value has been used twice already
        ...
        for i in range(dimen):
            taus.append(sum(order1[(order1.v1 == i) | (order1.v2 == i)].tauabs)) #calculate the sum of all taus for a specific variable
        i = np.where(np.array(taus) == max(taus))[0][0] # find where the sum of the taus is the highest to find the vairable to place in the center of the first tree
        order2 = order1[(order1.v1 == i) | (order1.v2 == i)].reset_index(drop=True) #select the rows that include this variable
    ...
    for i in range(len(order1)):
        ...
        u3=np.vstack((u1[:,v1i] , u1[:,v2i])).T #stacking the combination to fit copula to
    ...
    v2_k = []
    aics = []
    for i in range(len(order1)-1): #loop through the first tree to identify all possible combination of nodes
        ...
        for j in np.where(np.array([item == v1i for item in list(order1.v1[i+1:])]) | np.array([item == v1i for item in list(order1.v2[i+1:])])| np.array([item == v2i for item in list(order1.v1[i+1:])])| np.array([item == v2i for item in list(order1.v2[i+1:])]))[0] +i + 1: # see if possible connection between nodei and nodej
            ...
            s= max(set(lst), key=lst.count) #variable that is common in both parent nodes
            # parent node values
            ui1 = v1_1[:,i] 
            ui2 = v2_1[:,i]
            uj1 = v1_1[:,j]
            ...
            ktau.append(abs(st.kendalltau(v1igs ,  v2jgs  )[0])) # calculate the absolute kendall tau between v1igs and v2jgs, add it to the ktau list
    ...
    orderk =  pd.DataFrame({'v1': v1, 'v2': v2, 'tauabs': ktau, 'node': node}) # put the v1, v2, tauabs, and the node in list into a dataframe for tree k
    
    if vine == 'R' or vine == 'D':
        if len(orderk) > dimen-k:  # the tree will have the number of variables - k number of rows, if this is exceeded in orderk, a selection has to be made based on kendal tau
            orderk = orderk.sort_values(by='tauabs', ascending=False) # sort this dataframe from highest to lowest tauabs
            indexes = list(orderk.index) # create a list the original order of the dataframe prior to sorting
            orderk = orderk.reset_index(drop=True) # reset the index
            inde = []  #  list to put used rows of orderk in based on their oroginal position in the dataframe  
            inde2 = []  # list to put used rows of orderk i
            for i in range(len(orderk)):
                if i == 0:
                    order = orderk.head(1)  # loop through all the rows to include those pairs with the highest ktau first
                    inde.append(indexes[i])  # add used rows to inde (original row value)
                    ...
                    if (orderk.v1[i] in list(orderk.v2[:i]) or orderk.v1[i] in list(orderk.v1[:i]) ) and ( orderk.v2[i] in list(orderk.v2[:i]) or orderk.v2[i] in list(orderk.v1[:i])):
                        ...
                        order = pd.concat([order, orderk.loc[i].to_frame().T], ignore_index=True) #if conditions are met, add this to the dataframe of tree k
            ...
            if len(order) < (dimen-k):
                for i in range(1, len(orderk)):
                    ...
                    l1 = list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v1[i])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v1[i])])
                    lk = l1.copy()
                    while len(lk) > 0:
                        lk2 = lk.copy()
                        lk = []
                        for j in lk2:
                            ln =  list(order.v2.astype(str)[order.v1.astype(str) == j]) + list(order.v1.astype(str)[order.v2.astype(str) ==j])
                            try:
                    ...
                    l2 = list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v2[i])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v2[i])])
                    lk = l2.copy()
                    while len(lk) > 0:
                        lk2 = lk.copy()
                        lk = []
                    ...
                    if len(lst) == len(set(lst)):
                        inde.append(indexes[i]) # add used rows to inde (original row value)
                        order = pd.concat([order, orderk.loc[i].to_frame().T], ignore_index=True)  # if conditions are met, add this to the dataframe of tree k
                    if len(order) == dimen-k: #end loop when desired number of rows (number of variables - k) has been met
                        break
            ...
            orderk = orderk.sort_values(by='tauabs', ascending=False) # sort this dataframe from highest to lowest tauabs
            v1_k = np.array([v1_k[ind] for ind in orderk.index]).T #sort array of first node
            v2_k = np.array([v2_k[ind] for ind in orderk.index]).T #sort array of second node
            orderk = orderk.reset_index(drop=True)
            orderk['tree'] = k - 1
            v1_2 = v1_k.copy()
            v2_2 = v2_k.copy()
    ...
    if vine == 'C':
        if len(orderk) > dimen-k:
            subnodes = np.unique(np.stack((np.array(orderk.v1),np.array(orderk.v2)))) #see all the unique parent nodes
            taus =[] #list to put the sum of all tauabs in for a specific nodes
            for i in range(len(subnodes)):
                orderksub = orderk[(orderk['v1'].apply(lambda x: x == subnodes[i])) | (orderk['v2'].apply(lambda x: x == subnodes[i]))] 
                taus.append(sum(orderksub.tauabs)) #calculate the sum of all taus for a specific variable
            i = np.where(np.array(taus) == max(taus))[0][0] # find where the sum of the taus is the highest to find the vairable to place in the center of the first tree
            orderk = orderk[(orderk['v1'].apply(lambda x: x == subnodes[i])) | (orderk['v2'].apply(lambda x: x == subnodes[i]))] #select rows where this node is included
            orderk = orderk.sort_values(by='tauabs', ascending=False) # sort this dataframe from highest to lowest tauabs
    ...
    for i in range(len(order2)):
        u3=np.vstack((v1_2[:,i] , v2_2[:,i])).T #stacking the combination to fit copula to
        cop, rho, aic= bestcop(copsi, u3) # fit the best copula
        aics.append(aic) # add AIC to aics
        rhos.append(rho) # add parameters to rhos
        cops.append(cop) # add copula to cops
    ...
    if dimen > 3:
        for k in range(3,dimen):
            order = locals()['order' + str(k-1)].copy() #select the struture of the previous tree
            v1s =  locals()['v1_' + str(k-1)].copy() # select the first nodes of the previous tree
            v2s =  locals()['v2_' + str(k-1)].copy() # select the second nodes of the previous tree
            ...
            for i in range(len(order) - 1):
                ...
                for j in np.where(np.array([item == v1i for item in list(order.v1[i+1:])]) | np.array([item == v1i for item in list(order.v2[i+1:])])| np.array([item == v2i for item in list(order.v1[i+1:])])| np.array([item == v2i for item in list(order.v2[i+1:])]))[0] +i + 1:  # see if possible connection between nodei and nodej
                    v1i = order.v1[i].copy()  # parent node 1 from nodei in tree 
                    v2i = order.v2[i].copy()  # parent node 2 from nodei in tree 
            ...
            orderk =  pd.DataFrame({'v1': v1, 'v2': v2, 'tauabs': ktau, 'node': node, 'l': lk, 'r': rk}  ) # put information in dataframe for tree k
            ...
            if vine == 'R' or vine == 'D':
                if len(orderk) > dimen-k: # the tree will have the number of variables - k number of rows, if this is exceeded in orderk, a selection has to be made based on kendal tau
                    ...
                    for j in range(len(orderk)):
                        u3=np.vstack((v1_k[:,j] , v2_k[:,j])).T # stacking the combination to fit copula to
                        cop, rho , aic = bestcop(copsi, u3)  # fit the best copula
                        aics.append(aic) # add AIC to aics
                        rhos.append(rho) # add parameters to rhos
                        cops.append(cop) # add copula to cops
    ...
    for i in range(1,dimen):
        order = pd.concat([order, locals()['order' + str(i)]]).reset_index(drop=True) # adding trees in dataframe
    ...
    for i in list(range(dimen-1))[::-1]: #create the matix for the tree structure starting with the last tree
    ...
    for t in list(range(dimen-1)):
        ...
        if t == 0:
            ...
            for k in range(len(orderk)):
                ...
                v1igs = hfunc(copi, ui1, ui2, pari, un = uni) 
                v2jgs = hfunc(copj, uj1, uj2, parj, un = unj)
                u3=np.vstack((v1igs ,v2jgs)).T #stacking the combination to fit copula to
                cop, rho, aic = bestcop(copsi, u3) # fit the best copula
...
def conditionalvine(u1, vint, copsi, vine ='R', condition = 1, printing = True):
    ...
    if vine == 'R':
        ...
        if len(Y) > 1:
            for j in range(len(Y) - 1):
                ...
                for z in range(j+1, len(Y)):
                    if z == 1:
                        l = ((order1.v1 == Y[j]) & (order1.v2 == Y[z])) | ((order1.v1 == Y[z]) & (order1.v2 == Y[j]))
                    else: 
                        l = l | ((order1.v1 == Y[j]) & (order1.v2 == Y[z])) | ((order1.v1 == Y[z]) & (order1.v2 == Y[j]))
            inde = [] # list to put used rows of order1 in 
            l = np.where(l == True)[0]
            ...
            for k in l:
                ...
                lst = list(order2.v2[order2.v1 == order1.v2[k]]) + list(order2.v1[order2.v2 == order1.v2[k]]) +list(order2.v2[order2.v1 == order1.v1[k]]) + list(order2.v1[order2.v2 == order1.v1[k]]) 
                l1 = list(order2.v2[order2.v1 == order1.v2[k]]) + list(order2.v1[order2.v2 == order1.v2[k]])
                l2 = list(order2.v2[order2.v1 == order1.v1[k]]) + list(order2.v1[order2.v2 == order1.v1[k]])
                lk = l1.copy()
                while len(lk) > 0:
                    lk2 = lk.copy()
                    lk = []
                ...
                if skip == True:
                    continue
                if len(lst) == len(set(lst)):
                    order2 = pd.concat([order2, order1.loc[k].to_frame().T], ignore_index=True)
                    inde.append(k)
                    if len(order2) == len(Y) - 1:  #end loop when desired number of rows (number of variables in Y - 1) has been met
                        break
            ...
            for z in X:
                ...
                if z == X[0]:
                    l = ((order1.v1 == Y[0]) & (order1.v2 == z)) | ((order1.v1 == z) & (order1.v2 == Y[0]))
                else: 
                    l = l | ((order1.v1 == Y[0]) & (order1.v2 == z)) | ((order1.v1 == z) & (order1.v2 == Y[0]))
            inde = [] # list to put used rows of order1 in
            l = np.where(l == True)[0]
        ...
        while len(order2) < (dimen - 1): 
            ...
            if len(order2)  == dimen - 2 and len(X) > 1:
                ...
                Y3 = []
                for i in X3:
                    Y3 = Y3 + list(order2.v1[order2.v2 == i]) + list(order2.v2[order2.v1 == i]) + [i]

                Y3 = list(np.unique(Y3))
                for j in Y3:
                    for z in X2:
                        if j == Y3[0] and z == X2[0]:
                            l = ((order1.v1 == j) & (order1.v2 == z)) | ((order1.v1 == z) & (order1.v2 ==j))
                        else: 
                            l = l | ((order1.v1 == j) & (order1.v2 == z)) | ((order1.v1 == z) & (order1.v2 ==j))
            ...
            for k in l:
                ...
                if order1.v1[k] in X and order1.v2[k] in X:
                    ...
                    if order1.v2[k] in Y2:
                        ...
                        numapear = sum(((order2['v1'].apply(lambda x: order1.v1[k]== x )) | (order2['v2'].apply(lambda x: order1.v1[k]== x))))
                        for i in Y:
                            if numapear == sum(((order2['v1'].apply(lambda x: i== x )) | (order2['v2'].apply(lambda x: i== x)))):
                                skip = True
                                break
        ...
        for i in Y:
            taus.append(sum(order1[(order1.v1 == i) | (order1.v2 == i)].tauabs)) #calculate the sum of all taus for a specific variable
        i = Y[np.where(np.array(taus) == max(taus))[0][0]] # find where the sum of the taus is the highest to find the vairable to place in the center of the first tree
        order2 = order1[(order1.v1 == i) | (order1.v2 == i)].reset_index(drop=True) #select the rows that include this variable
        order2['both_in_Y'] = order2.apply(lambda row: (row['v1'] in Y) and (row['v2'] in Y), axis=1) #find the columns with values in Y
        order2 = order2.sort_values(by='both_in_Y', ascending=False).reset_index(drop=True) # Sort the DataFrame by the 'both_in_Y' column
    ...
    if vine == 'R':
        if len(orderk) > dimen-k:
            ...
            orderk = orderk.sort_values(by='tauabs', ascending=False) # sort this dataframe from highest to lowest tauabs
            indexi = list(orderk.index)  # create a list the original order of the dataframe prior to sorting
            orderk = orderk.reset_index(drop=True) # reset the index
            inde = []  # list to put used rows of orderk i
            inde2 = []  #  list to put used rows of orderk in based on their oroginal position in the dataframe  
            Y3 = Y2.copy()
            X3 = X2.copy()
            ...
            # define all rows in orderk where the nodes in X are connected to nodes in Y
                for j in Y2:
                    for z in X2:
                        if j == Y2[0] and z == X2[0]:
                            l = ((orderk['v1'].apply(lambda x: j== x )) & (orderk['v2'].apply(lambda x: z== x))) | ((orderk['v1'].apply(lambda x:  z== x)) & (orderk['v2'].apply(lambda x: j== x)))
                        else: 
                            l = l |((orderk['v1'].apply(lambda x: j== x )) & (orderk['v2'].apply(lambda x: z== x))) | ((orderk['v1'].apply(lambda x:  z== x)) & (orderk['v2'].apply(lambda x: j== x)))
                ...
                for j in range(len(Y2) - 1):
                    ...
                    for z in range(j+1, len(Y2)):
                        if z == 1:
                            l = ((orderk['v1'].apply(lambda x: Y2[j]== x )) & (orderk['v2'].apply(lambda x:  Y2[z]== x))) | ((orderk['v1'].apply(lambda x:  Y2[z]== x)) & (orderk['v2'].apply(lambda x: Y2[j]== x)))
                        else: 
                            l = l |((orderk['v1'].apply(lambda x:  Y2[j]== x)) & (orderk['v2'].apply(lambda x:  Y2[z]== x))) | ((orderk['v1'].apply(lambda x:  Y2[z]== x)) & (orderk['v2'].apply(lambda x: Y2[j]== x)))
                ...
                for k in l:
                    if k == l[0]:  # add first one to the tree
                        ...
                        inde.append(k)
                        inde2.append(indexi[k]) # add used rows to inde (original row value)
                    if k in inde:  #check if row has not been added before
                        continue
                    lst = list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v1[k])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v1[k])])+ list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v2[k])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v2[k])])
                    l1 = list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v1[k])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v1[k])])
                    lk = l1.copy()
                    while len(lk) > 0:
                        lk2 = lk.copy()
                        lk = []
                    ...
                    l2 = list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v2[k])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v2[k])])
                    lk = l2.copy()
                    while len(lk) > 0:
                        lk2 = lk.copy()
                        lk = []
                    ...
                    if skip == True:
                       ...
                       #continue 
                    if len(lst) == len(set(lst)):
                        order = pd.concat([order, orderk.loc[k].to_frame().T], ignore_index=True)
                        inde.append(k)
                        inde2.append(indexi[k])
                        if len(order) == len(Y2) - 1: #end loop when desired number of rows (number of variables in Y - 1) has been met
                            break
        ...
        if len(orderk) > dimen-k:
            subnodes = order1[['v1', 'v2']].values.tolist() #see all the unique parent nodes
            taus =[] #list to put the sum of all tauabs in for a specific nodes
            ...
            if vine == 'R':
                ...
                if len(orderk) > dimen-k:
                    ...
                    if len(Y2) > 1:
                        ...
                        for z in l:
                            ...
                            if z in inde:
                                continue
                            lst = list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v1[z])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v1[z])])+ list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v2[z])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v2[z])])
                            l1 = list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v1[z])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v1[z])])
                            lk = l1.copy()
                            while len(lk) > 0:
                                lk2 = lk.copy()
                                lk = []
                            ...
                            l2 = list(order.v2.astype(str)[order.v1.astype(str) == str(orderk.v2[z])]) + list(order.v1.astype(str)[order.v2.astype(str) == str(orderk.v2[z])])
                            lk = l2.copy()
                            while len(lk) > 0:
                                lk2 = lk.copy()
                                lk = []
                ...
                if len(orderk) > dimen-k:
                    ...
                    if (len(Y) + 1 - k) > 1:
                        ...
                        orderk = orderk[(orderk['v1'].apply(lambda x: x == subnodes[0])) | (orderk['v2'].apply(lambda x: x == subnodes[0]))]
...
def plotvine(a, plottitle = None, variables = None, savepath = None):
    ...
    for t, ax in zip(range(1, dimen), axes.flat):  # Iterate through subplots and loop indices
        if t == 1:
            orderk =  locals()['order' + str(t)]
            edges = list(orderk.node)
            edges = [tuple(sublist) for sublist in edges]
            edge_labels = {edge: ','.join(map(str, orderk.node[i])) for i, edge in enumerate(edges)}
        elif t == 2:
            orderk =  locals()['order' + str(t)]
            edges = [(','.join(map(str, orderk.v1[i])), ','.join(map(str, orderk.v2[i]))) for i in range(len(orderk))]
